...
class Version_1_1_0 extends AbstractMigration
{
    ...
    protected function up(): void
    {
        ...
        $campaignsTable       = $this->concatPrefix('campaigns');
        $campaignEventsTable  = $this->concatPrefix('campaign_events');
        $contactsTable        = $this->concatPrefix('leads');
        $conditionResultTable = $this->concatPrefix('campaign_entity_condition_result');

        // 1. campaign_entity_condition_result - stores filter results from condition nodes
        if (!$schema->hasTable($conditionResultTable)) {
            $this->addSql(<<<SQL
CREATE TABLE {$conditionResultTable} (
    ...
    CONSTRAINT fk_cecr_campaign FOREIGN KEY (campaign_id)
        REFERENCES {$campaignsTable}(id) ON DELETE CASCADE,
    CONSTRAINT fk_cecr_campaign_event FOREIGN KEY (campaign_event_id)
        REFERENCES {$campaignEventsTable}(id) ON DELETE CASCADE,
    CONSTRAINT fk_cecr_contact FOREIGN KEY (contact_id)
        REFERENCES {$contactsTable}(id) ON DELETE CASCADE
...
SQL
        ...
        $conditionResultItemTable = $this->concatPrefix('campaign_entity_condition_result_item');
        if (!$schema->hasTable($conditionResultItemTable)) {
            $this->addSql(<<<SQL
CREATE TABLE {$conditionResultItemTable} (
    result_id INT NOT NULL,
    entity_id INT NOT NULL,
    PRIMARY KEY(result_id, entity_id),
    CONSTRAINT fk_cecri_result FOREIGN KEY (result_id)
        REFERENCES {$conditionResultTable}(id) ON DELETE CASCADE
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci ENGINE = InnoDB
SQL
            );
        }
        ...
        $sendLogTable = $this->concatPrefix('postmark_entity_send_log');
        if (!$schema->hasTable($sendLogTable)) {
            $this->addSql(<<<SQL
CREATE TABLE {$sendLogTable} (
    id INT AUTO_INCREMENT NOT NULL,
