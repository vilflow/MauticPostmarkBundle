 ...
 */
class Version20251102000000 extends PreUpAssertionMigration
{
    protected function preUpAssertions(): void
    {
        // Skip if the main table already exists
        $this->skipAssertion(
            function (Schema $schema) {
                return $schema->hasTable("{$this->prefix}postmark_entity_send_log");
            },
            'Per-entity send tables already present.'
        );
    }
    ...
    public function up(Schema $schema): void
    {
        ...
        if (!$schema->hasTable("{$this->prefix}campaign_entity_condition_result")) {
            $this->addSql(<<<SQL
CREATE TABLE {$this->prefix}campaign_entity_condition_result (
    ...
    CONSTRAINT fk_cecr_campaign FOREIGN KEY (campaign_id)
        REFERENCES {$this->prefix}campaigns(id) ON DELETE CASCADE,
    CONSTRAINT fk_cecr_campaign_event FOREIGN KEY (campaign_event_id)
        REFERENCES {$this->prefix}campaign_events(id) ON DELETE CASCADE,
    CONSTRAINT fk_cecr_contact FOREIGN KEY (contact_id)
        REFERENCES {$this->prefix}leads(id) ON DELETE CASCADE
...
SQL
        ...
        if (!$schema->hasTable("{$this->prefix}campaign_entity_condition_result_item")) {
            $this->addSql(<<<SQL
CREATE TABLE {$this->prefix}campaign_entity_condition_result_item (
    result_id INT NOT NULL,
    entity_id INT NOT NULL,
    PRIMARY KEY(result_id, entity_id),
    CONSTRAINT fk_cecri_result FOREIGN KEY (result_id)
        REFERENCES {$this->prefix}campaign_entity_condition_result(id) ON DELETE CASCADE
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci ENGINE = InnoDB
SQL
            );
        }
        ...
        if (!$schema->hasTable("{$this->prefix}postmark_entity_send_log")) {
            $this->addSql(<<<SQL
CREATE TABLE {$this->prefix}postmark_entity_send_log (
    id INT AUTO_INCREMENT NOT NULL,
